...
def find_possible_eff_sols(M):
    ...
    B = tools.Basis(M.B_indb)#,deepcopy(M.B_inv))
    for i in range(len(ind)):
        B.pivots.append( (int(pivot_outs[i]), int(M.B_ind[pivot_outs[i]]), int(M.N_ind[pivot_ins[i]]) ) )
...
def simplex(A,b,C, std_form = True, Initial_basic = None, num_sol = np.inf):
    ...
    while explore:
        ...
        pivots = (-1,-1)
        explored_bases.append(B_indb)
        tmp_new_explore = True
        valid = False
        while not valid:
            try:
                pivots = bases_explore[0].pivots.popleft()
                B_indb = (bases_explore[0].B_indb & ~(1 << pivots[1])) | (1 << pivots[2])
                if(B_indb not in explored_bases):
                    tmp_new_explore = False
                    valid = True
        ...
        if new_explore:
            ...
            print(f"Current B_ind: {M.B_ind}, trying to update to {tools.bin2ind(B_indb,num_variables)}")
            M.B_inv.revert2backup()
            M.update2(B_indb, pivots)
            print(f"Updated to {M.B_ind}")
        new_explore = tmp_new_explore
